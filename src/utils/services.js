...
const Article = `${GITHUB_API}/${username}/${repository}`
...
export const queryPosts = ({ page = 1, pageSize = 10, filter = '' }) => {
  const url = `${Article}/issues?state=open&page=${page}&per_page=${pageSize}${filter}`
  return githubFetch(url)
}

// 获取单篇文章
export const queryPost = number => {
  const url = `${Article}/issues/${number}?state=open`
  return githubFetch(url)
}

// 获取分类
export const queryCategory = () => {
  const url = `${Article}/milestones`
  return githubFetch(url)
}

// 获取标签
export const queryTag = () => {
  const url = `${Article}/labels?page=1&per_page=100`
  return githubFetch(url)
}

// 获取灵感
export const queryInspiration = ({ page = 1, pageSize = 10 }) => {
  const url = `${Article}/issues?state=closed&labels=inspiration&page=${page}&per_page=${pageSize}`
  return githubFetch(url)
}
...
export const queryPage = type => {
  ...
  const url = `${Article}/issues?state=closed&labels=${upperType}`
